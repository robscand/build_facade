...
class PrintList:
    def __init__(self):
        ...
        self.headerFont = QtGui.QFont("Arial", pointSize = 10, weight = QtGui.QFont.Medium)
        self.bodyFont = QtGui.QFont("Arial", pointSize = 10)
        self.footerFont = QtGui.QFont("Arial", pointSize = 9, italic = True)
        self.headerFlags = QtCore.Qt.AlignLeft | QtCore.Qt.TextWordWrap
        self.bodyFlags = QtCore.Qt.TextWordWrap
        self.footerFlags = QtCore.Qt.AlignLeft | QtCore.Qt.TextWordWrap
        color = QtGui.QColor(QtCore.Qt.black)
        self.headerPen = QtGui.QPen(color, 1.25)
    ...
    def printRow(self, widths, cellData, height, flags):
        ...
        for i in range(len(widths)):
            ...
            widths[i] - self.margin, height - 2 * self.margin, flags,
            str(cellData[i]))
    ...
    def _printFooterRow(self):
        ...
        self._painter.drawText(self.margin,
                               self.printer.height() - self._footerRowHeight - self.margin,
                               self.printer.width() - 2 * self.margin,
                               self._footerRowHeight - 2 * self.margin,
                               self.footerFlags,
                               str(self._currentPageNumber))
